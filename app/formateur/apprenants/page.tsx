...
export default function ApprenantPage() {
  ...
  return (
    <div>
      <div style={{marginBottom:'2rem',padding:'1.5rem',background:'linear-gradient(135deg,rgba(123,92,245,0.1),rgba(34,212,168,0.05))',border:'1px solid rgba(123,92,245,0.2)',borderRadius:'20px'}}>
        <h1 style={{fontSize:'24px',fontWeight:'800',background:'linear-gradient(135deg,#F0EEFF,#A78BF8)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>Mes apprenants</h1>
        <p style={{color:'#8B7BAE',fontSize:'13px',marginTop:'3px'}}>{apprenants.length} apprenants · Progression moyenne {avg(apprenants.map(a=>a.progress))}% · Score moyen {avg(apprenants.map(a=>a.score))}/100</p>
      </div>
      ...
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Rechercher un apprenant ou un cours..."
        style={{width:'100%',background:'rgba(123,92,245,0.06)',color:'#F0EEFF',border:'1px solid rgba(123,92,245,0.2)',borderRadius:'12px',padding:'11px 16px',fontSize:'14px',fontFamily:'inherit',outline:'none',marginBottom:'1.5rem'}} />
          ...
          <tbody>
            {filtered.map((a,i)=>(
              <tr key={a.id} style={{borderBottom:i<filtered.length-1?'1px solid rgba(123,92,245,0.07)':'none',transition:'background .15s'}}
                ...
                <td style={{padding:'13px 14px',fontSize:'12px',color:'#8B7BAE',maxWidth:'140px'}}><div style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.cours}</div></td>
                <td style={{padding:'13px 14px'}}>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'1rem'}}>
          <div style={{background:'linear-gradient(145deg,#1A1530,#130F23)',border:'1px solid rgba(123,92,245,0.25)',borderRadius:'20px',padding:'2rem',width:'100%',maxWidth:'480px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.5rem'}}>
              ...
              <button onClick={()=>setSel(null)} style={{background:'none',border:'none',color:'#4A3D6A',fontSize:'18px',cursor:'pointer'}}>✕</button>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.75rem',marginBottom:'1.5rem'}}>
              {[
                ...
                <div key={s.l} style={{background:'rgba(0,0,0,0.2)',borderRadius:'10px',padding:'12px'}}>
            ...
            <div style={{background:'rgba(123,92,245,0.06)',border:'1px solid rgba(123,92,245,0.15)',borderRadius:'12px',padding:'14px',marginBottom:'1rem'}}>
              ...
              <textarea placeholder={`Envoyer un message de suivi à ${sel.name}...`} rows={3} style={{width:'100%',background:'rgba(0,0,0,0.2)',color:'#F0EEFF',border:'1px solid rgba(123,92,245,0.15)',borderRadius:'8px',padding:'8px 12px',fontSize:'13px',fontFamily:'inherit',outline:'none',resize:'none'}} />
