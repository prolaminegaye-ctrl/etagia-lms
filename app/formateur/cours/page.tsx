  ...
  return (
    <div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
        {cours.map(c=>(
          <div key={c.id} style={{background:'#FFFFFF',border:'1px solid rgba(28,25,23,0.07)',borderRadius:'18px',padding:'1.5rem',transition:'all .2s'}}>
            <div style={{display:'flex',alignItems:'flex-start',gap:'1.5rem'}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{display:'flex',gap:'12px',flexWrap:'wrap',marginBottom:'1rem'}}>
                  {[
                    ...
                    <div key={x.l}>
                      <span style={{fontSize:'10px',color:'#57534E',textTransform:'uppercase',letterSpacing:'0.5px'}}>{x.l} </span>
                      <span style={{fontSize:'12px',color:'#A8A29E',fontWeight:'500'}}>{x.v}</span>
                    </div>
                  ))}
                </div>
                {c.status==='Publié'&&(
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'0.75rem'}}>
                    {[
                      {l:'Apprenants inscrits',v:c.enrolled,color:'#E8651A'},
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'8px',minWidth:'160px'}}>
                ...
                {c.status==='Publié'&&(
                  <>
                    <button onClick={()=>router.push(`/formateur/creer`)} style={{background:'rgba(28,25,23,0.06)',border:'1px solid rgba(28,25,23,0.10)',borderRadius:'10px',padding:'9px 16px',color:'#E8651A',fontSize:'13px',fontWeight:'600',cursor:'pointer',textAlign:'center'}}>✏️ Modifier</button>
