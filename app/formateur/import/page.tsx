...
const typeColors: Record<string,string> = { SCORM:'#FF5722', H5P:'#FFB300', PDF:'#F05A5A', Vidéo:'#00BFA5', HTML:'#FFB300', Fichier:'#8B7BAE' }
  ...
  return (
    <div>
      ...
      <div onClick={()=>inputRef.current?.click()} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);handleFiles(e.dataTransfer.files)}}
        style={{border:`2px dashed ${drag?'#FF5722':'rgba(123,92,245,0.2)'}`,borderRadius:'20px',padding:'3rem 2rem',textAlign:'center',background:drag?'rgba(123,92,245,0.06)':'linear-gradient(145deg,#1A1530,#130F23)',cursor:'pointer',marginBottom:'1.5rem',transition:'all .2s'}}>
