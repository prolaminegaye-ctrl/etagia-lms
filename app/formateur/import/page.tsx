...
type FileInfo = { name: string; size: string; type: string; url: string; id: string }

const formatSize = (b: number) => b < 1024*1024 ? `${(b/1024).toFixed(0)} KB` : `${(b/1024/1024).toFixed(1)} MB`
const getType = (name: string) => {
  const ext = name.split('.').pop()?.toLowerCase()
  if (['zip'].includes(ext||'')) return 'SCORM'
...
export default function ImportPage() {
  ...
  const processFile = async (file: File) => {
    ...
    const info: FileInfo = { id, name: file.name, size: formatSize(file.size), type: getType(file.name), url: URL.createObjectURL(file) }
  ...
  const formats = [
    ...
    { f:'HTML/ZIP', d:'Pages web et micro-modules', i:'💻', c:'#F0B429', ext:'.html .zip' },
    { f:'PDF', d:'Documents et supports', i:'📄', c:'#F05A5A', ext:'.pdf' },
    { f:'Vidéo MP4', d:'Cours vidéo haute qualité', i:'🎬', c:'#22D4A8', ext:'.mp4 .mov' },
    { f:'xAPI / Tin Can', d:'Expériences d\'apprentissage', i:'🔗', c:'#8B7BAE', ext:'.zip' },
  ]

  return (
    <div>
      <div style={{ marginBottom: '2rem', padding: '1.5rem', background: 'linear-gradient(135deg,rgba(123,92,245,0.12),rgba(224,64,160,0.06))', border: '1px solid rgba(123,92,245,0.25)', borderRadius: '20px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '800', marginBottom: '4px', background: 'linear-gradient(135deg,#F0EEFF,#A78BF8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>↑ Importer un contenu</h1>
        <p style={{ color: '#8B7BAE', fontSize: '14px' }}>SCORM 1.2 · SCORM 2004 · xAPI · H5P · HTML · PDF · Vidéo</p>
      </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: done[f.id] ? '0' : '10px' }}>
                ...
                {done[f.id] ? (
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    {f.type === 'PDF' && <a href={f.url} target="_blank" rel="noreferrer" style={{ background: 'rgba(123,92,245,0.12)', border: '1px solid rgba(123,92,245,0.25)', borderRadius: '8px', padding: '5px 12px', color: '#A78BF8', fontSize: '12px', fontWeight: '700' }}>👁 Voir</a>}
                    {f.type === 'Vidéo' && <a href={f.url} target="_blank" rel="noreferrer" style={{ background: 'rgba(34,212,168,0.12)', border: '1px solid rgba(34,212,168,0.25)', borderRadius: '8px', padding: '5px 12px', color: '#22D4A8', fontSize: '12px', fontWeight: '700' }}>▶ Lire</a>}
                    {f.type === 'SCORM' && <span style={{ background: 'rgba(123,92,245,0.12)', border: '1px solid rgba(123,92,245,0.25)', borderRadius: '8px', padding: '5px 12px', color: '#A78BF8', fontSize: '12px', fontWeight: '700' }}>📦 SCORM prêt</span>}
                    <button onClick={() => remove(f.id)} style={{ background: 'rgba(240,90,90,0.1)', border: 'none', borderRadius: '8px', padding: '5px 10px', color: '#F05A5A', fontSize: '12px', cursor: 'pointer' }}>✕</button>
                  </div>
          ...
          {files.some(f => done[f.id]) && (
            <div style={{ background: 'rgba(34,212,168,0.06)', border: '1px solid rgba(34,212,168,0.2)', borderRadius: '14px', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#8B7BAE' }}>Prêts à être intégrés dans vos cours</div>
              </div>
              <a href="/formateur/cours" style={{ marginLeft: 'auto', background: 'linear-gradient(135deg,#7B5CF5,#E040A0)', borderRadius: '10px', padding: '8px 18px', color: '#fff', fontSize: '13px', fontWeight: '700' }}>Gérer mes cours →</a>
            </div>
          )}
        </div>
      )}
      ...
      {/* Supported formats */}
      <h2 style={{ fontSize: '16px', fontWeight: '700', marginBottom: '1rem', color: '#F0EEFF' }}>Formats supportés</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '0.75rem' }}>
        {formats.map(f => (
          <div key={f.f} style={{ background: 'linear-gradient(145deg,#1A1530,#130F23)', border: '1px solid rgba(123,92,245,0.1)', borderRadius: '14px', padding: '1rem', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '28px', marginBottom: '8px', marginTop: '4px' }}>{f.i}</div>
            <div style={{ fontWeight: '700', fontSize: '13px', color: f.c, marginBottom: '4px' }}>{f.f}</div>
            <div style={{ fontSize: '12px', color: '#8B7BAE', marginBottom: '6px' }}>{f.d}</div>
            <div style={{ fontSize: '11px', color: '#4A3D6A', fontFamily: 'monospace' }}>{f.ext}</div>
          </div>
        ))}
      </div>
    </div>
