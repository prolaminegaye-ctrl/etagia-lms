...
export default function AdaptivePage() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: '26px', fontWeight: '700', marginBottom: '4px', color: 'var(--ink)' }}>🧠 Parcours Adaptatif</h1>
      <p style={{ color: 'var(--ink-mut)', fontSize: '14px', marginBottom: '2rem' }}>L&apos;IA adapte votre parcours selon vos performances en temps réel</p>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
        {[
          { label: 'Modules complétés', value: `${completed}/${learningPath.length}`, color: 'var(--turq)' },
          { label: 'Score moyen', value: `${Math.round(avgScore)}%`, color: 'var(--orange)' },
          { label: 'Niveau actuel', value: 'Intermédiaire', color: 'var(--gold)' },
        ].map(s => (
          <div key={s.label} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '1.25rem', textAlign: 'center' }}>
            <div style={{ fontSize: '28px', fontWeight: '800', color: s.color, fontFamily: 'var(--font-display)', marginBottom: '4px' }}>{s.value}</div>
            <div style={{ fontSize: '12px', color: 'var(--ink-mut)' }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* AI insight */}
      <div style={{ background: 'var(--turq-50)', border: '1px solid var(--turq-300)', borderRadius: '16px', padding: '1.25rem', marginBottom: '2rem', display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
        <div style={{ fontSize: '24px', flexShrink: 0 }}>✦</div>
        <div>
          <div style={{ fontWeight: '700', color: 'var(--turq-700)', marginBottom: '6px', fontSize: '14px' }}>Recommandation IA personnalisée</div>
          <p style={{ fontSize: '13px', color: 'var(--ink-mut)', lineHeight: '1.6' }}>
            Votre score de <strong style={{ color: 'var(--ink)' }}>78% en structures de données</strong> suggère de consolider les bases avant d&apos;avancer. Je recommande de revoir le module 2 avant Pandas. Vous progressez <strong style={{ color: 'var(--turq)' }}>23% plus vite</strong> que la moyenne des apprenants de votre niveau.
          </p>
        </div>
      </div>
        ...
        {learningPath.map((item, i) => (
          <div key={item.id}>
            <div onClick={() => setSelected(selected === item.id ? null : item.id)} style={{
              background: item.status === 'current' ? 'var(--turq-50)' : 'var(--surface)',
              border: `1px solid ${item.status === 'current' ? 'var(--turq-300)' : 'var(--line)'}`,
              ...
              <div style={{ width: '38px', height: '38px', borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px',
                ...
                color: item.status === 'completed' ? 'var(--turq)' : item.status === 'current' ? 'var(--orange)' : 'var(--text-muted)'
              }}>
                {item.status === 'completed' ? '✓' : item.status === 'current' ? '▶' : '🔒'}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: '600', fontSize: '14px', color: 'var(--ink)', marginBottom: '3px' }}>{item.title}</div>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  {item.score > 0 && <span style={{ fontSize: '11px', color: 'var(--turq)', fontWeight: '600' }}>Score: {item.score}%</span>}
                </div>
              </div>
            ...
            {i < learningPath.length - 1 && (
              <div style={{ width: '2px', height: '10px', background: 'var(--line)', margin: '0 auto', marginLeft: '28px' }} />
            )}
          </div>
        ))}
      </div>
