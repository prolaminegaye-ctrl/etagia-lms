...
export default function DiagnosticPage() {
  ...
  const getProfile = () => {
    ...
    if (techScore >= 2) return { label: 'Profil Tech', color: 'var(--orange)', courses: ['Data Science avec Python', 'IA Générative pour pros', 'Excel Pro'] }
    if (bizScore >= 2) return { label: 'Profil Business', color: 'var(--turq)', courses: ['Marketing Digital Afrique', 'Leadership & Management', 'Pitch & Fundraising'] }
  ...
  if (submitted) {
    ...
    return (
      <div>
        <h1 style={{ fontSize: '26px', fontWeight: '700', marginBottom: '0.5rem', color: 'var(--ink)' }}>🎯 Votre profil d&apos;apprentissage</h1>
        <p style={{ color: 'var(--ink-mut)', fontSize: '14px', marginBottom: '2rem' }}>Basé sur votre évaluation diagnostique</p>

        <div style={{ background: 'var(--surface)', border: `1px solid ${profile.color}50`, borderRadius: '20px', padding: '2rem', marginBottom: '1.5rem', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--ink-mut)', fontSize: '14px' }}>Voici les cours recommandés pour vous par notre IA adaptive</p>
        </div>

        <h2 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '1rem', color: 'var(--ink)' }}>Parcours recommandé</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '2rem' }}>
          {profile.courses.map((c, i) => (
            <div key={c} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ fontWeight: '500', color: 'var(--ink)' }}>{c}</span>
              <a href="/cours" style={{ marginLeft: 'auto', color: 'var(--orange)', fontSize: '13px', fontWeight: '600' }}>Commencer →</a>
            </div>
          ))}
        </div>
  ...
  return (
    <div>
      <h1 style={{ fontSize: '26px', fontWeight: '700', marginBottom: '4px', color: 'var(--ink)' }}>📋 Évaluation diagnostique</h1>
      <p style={{ color: 'var(--ink-mut)', fontSize: '14px', marginBottom: '2rem' }}>Aidez-nous à personnaliser votre parcours d&apos;apprentissage</p>

      {/* Progress */}
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '13px', color: 'var(--ink-mut)' }}>
          <span>Question {current + 1} / {questions.length}</span>
          <span style={{ color: 'var(--orange)', fontWeight: '600' }}>{Math.round(progress)}%</span>
        </div>
      ...
      {/* Question */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--turq-300)', borderRadius: '20px', padding: '2rem', marginBottom: '1.5rem' }}>
        <div style={{ fontSize: '11px', color: 'var(--orange)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px' }}>{q.cat}</div>
        <h2 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '1.5rem', color: 'var(--ink)', lineHeight: '1.4' }}>{q.q}</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {q.opts.map((opt, i) => (
            <button key={i} onClick={() => answer(q.id, i)} style={{
              background: answers[q.id] === i ? 'var(--accent-muted)' : 'var(--bg-secondary)',
              border: `1px solid ${answers[q.id] === i ? 'var(--turq-300)' : 'var(--line)'}`,
              borderRadius: '12px', padding: '14px 18px', color: answers[q.id] === i ? 'var(--orange)' : 'var(--ink)',
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'space-between' }}>
        {current > 0 && <button onClick={() => setCurrent(c => c-1)} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '10px', padding: '10px 20px', color: 'var(--ink-mut)', fontSize: '14px' }}>← Précédent</button>}
