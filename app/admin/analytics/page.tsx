    ...
    <div>
      ...
      <div style={{ borderRadius: '24px', padding: '2rem 2.5rem', marginBottom: '2rem', background: 'linear-gradient(135deg, #F4591F 0%, #FF8C42 45%, #FFB347 100%)', position: 'relative', overflow: 'hidden', boxShadow: '0 8px 32px rgba(244,89,31,0.25)' }}>
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ fontSize: '10px', fontWeight: '800', color: 'rgba(255,255,255,0.7)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '6px' }}>Analytics</div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: '900', color: '#fff', letterSpacing: '-0.4px', marginBottom: '4px' }}>Analytics & KPIs</h1>
          <p style={{ color: 'rgba(255,255,255,0.78)', fontSize: '14px' }}>Suivez les performances de la plateforme.</p>
        </div>
      </div>
      <h1 style={{ fontSize:'26px', fontWeight:'700', marginBottom:'2rem' }}>Analytics plateforme</h1>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1rem', marginBottom:'2rem' }}>
        {[
          { label:'Nouveaux utilisateurs (mai)', value:'+127', color:'var(--orange)' },
          { label:'Sessions actives aujourd\'hui', value:'348', color:'var(--turq)' },
          { label:'Taux de rétention', value:'84%', color:'var(--gold)' },
        ].map(s => (
          <div key={s.label} style={{ background:'var(--gradient-card)', border:'1px solid var(--line)', borderRadius:'16px', padding:'1.5rem', boxShadow:'var(--shadow-card)' }}>
            <div style={{ fontSize:'32px', fontWeight:'800', color:s.color, fontFamily:'var(--font-display)', marginBottom:'6px' }}>{s.value}</div>
            <div style={{ fontSize:'13px', color:'var(--ink-mut)' }}>{s.label}</div>
          </div>
        ))}
      </div>
      <div style={{ background:'var(--gradient-card)', border:'1px solid var(--line)', borderRadius:'16px', padding:'1.5rem', boxShadow:'var(--shadow-card)' }}>
        <h2 style={{ fontSize:'16px', fontWeight:'600', marginBottom:'1.5rem' }}>Inscriptions mensuelles</h2>
        <div style={{ display:'flex', alignItems:'flex-end', gap:'12px', height:'160px' }}>
          {data.map((v,i) => (
            <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'8px' }}>
              <div style={{ fontSize:'12px', fontWeight:'600', color:'var(--orange)' }}>{v}</div>
