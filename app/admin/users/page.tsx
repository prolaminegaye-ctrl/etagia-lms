  ...
  return (
    <div>
      <div style={{marginBottom:'2rem',padding:'1.5rem',background:'linear-gradient(135deg,rgba(123,92,245,0.1),rgba(34,212,168,0.05))',border:'1px solid rgba(123,92,245,0.2)',borderRadius:'20px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
          <h1 style={{fontSize:'24px',fontWeight:'800',background:'linear-gradient(135deg,#F0EEFF,#A78BF8)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>Utilisateurs</h1>
          <p style={{color:'#8B7BAE',fontSize:'13px',marginTop:'3px'}}>{users.length} utilisateurs · {users.filter(u=>u.status==='Actif').length} actifs</p>
        </div>
      ...
      <div style={{display:'flex',gap:'10px',marginBottom:'1.5rem',flexWrap:'wrap'}}>
        ...
        {['Tous','Admin','Formateur','Apprenant'].map(r=>(
          <button key={r} onClick={()=>setFilterRole(r)} style={{padding:'8px 16px',borderRadius:'20px',fontSize:'12px',fontWeight:'600',border:`1px solid ${filterRole===r?'rgba(123,92,245,0.5)':'rgba(123,92,245,0.15)'}`,background:filterRole===r?'rgba(123,92,245,0.15)':'transparent',color:filterRole===r?'#A78BF8':'#8B7BAE',cursor:'pointer'}}>
            {r}
          </button>
        ))}
      </div>
      ...
      <div style={{background:'linear-gradient(145deg,#1A1530,#130F23)',border:'1px solid rgba(123,92,245,0.12)',borderRadius:'16px',overflow:'hidden'}}>
              ...
              <tr key={u.id} style={{borderBottom:i<filtered.length-1?'1px solid rgba(123,92,245,0.07)':'none',transition:'background .15s'}}
                ...
                <td style={{padding:'14px'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                    ...
                    <div>
                      <div style={{fontWeight:'500',fontSize:'13px',color:'#F0EEFF'}}>{u.name}</div>
                      <div style={{fontSize:'11px',color:'#4A3D6A'}}>{u.email}</div>
                    </div>
                  </div>
                </td>
                <td style={{padding:'14px'}}>
                ...
                <td style={{padding:'14px'}}>
                  <div style={{display:'flex',gap:'5px'}}>
                    <button onClick={()=>openEdit(u)} style={{background:'rgba(123,92,245,0.1)',border:'none',borderRadius:'6px',padding:'5px 10px',color:'#A78BF8',fontSize:'11px',fontWeight:'600',cursor:'pointer'}}>✏️ Éditer</button>
        ...
        {filtered.length===0&&<div style={{padding:'3rem',textAlign:'center',color:'#4A3D6A'}}>Aucun utilisateur trouvé</div>}
      </div>
      ...
      {(modal==='add'||modal==='edit')&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'1rem'}}>
          <div style={{background:'linear-gradient(145deg,#1A1530,#130F23)',border:'1px solid rgba(123,92,245,0.3)',borderRadius:'20px',padding:'2rem',width:'100%',maxWidth:'500px'}}>
            ...
            {modal==='add'&&(
              <div style={{background:'rgba(123,92,245,0.06)',border:'1px solid rgba(123,92,245,0.15)',borderRadius:'10px',padding:'10px 14px',marginBottom:'1.5rem',fontSize:'12px',color:'#8B7BAE'}}>
      ...
      {modal==='delete'&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}}>
          <div style={{background:'linear-gradient(145deg,#1A1530,#130F23)',border:'1px solid rgba(240,90,90,0.3)',borderRadius:'20px',padding:'2.5rem',textAlign:'center',maxWidth:'380px',margin:'1rem'}}>
            ...
            <p style={{color:'#8B7BAE',fontSize:'13px',marginBottom:'2rem'}}>L'utilisateur perdra l'accès à tous ses cours et ses données seront supprimées.</p>
