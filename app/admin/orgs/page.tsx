...
const planColors: Record<string,string> = { Enterprise: '#FFB300', Pro: '#5B8DEF', Starter: '#00BFA5' }
...
export default function OrgsPage() {
  ...
  const inp: React.CSSProperties = { background:'#FAF9F7', color:'#1C1917', border:'1px solid rgba(28,25,23,0.09)', borderRadius:'10px', padding:'9px 12px', width:'100%', fontSize:'14px', fontFamily:'inherit', outline:'none' }
  const lbl: React.CSSProperties = { fontSize:'11px', color:'#A8A29E', display:'block', marginBottom:'5px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'0.7px' }
  const btnPrimary: React.CSSProperties = { background:'linear-gradient(135deg,#5B8DEF,#00BFA5)', border:'none', borderRadius:'10px', padding:'10px 20px', color:'#fff', fontWeight:'700', fontSize:'14px', cursor:'pointer' }
  const btnCancel: React.CSSProperties = { background:'#FAF9F7', border:'1px solid rgba(28,25,23,0.09)', borderRadius:'10px', padding:'10px 18px', color:'#A8A29E', fontSize:'14px', cursor:'pointer' }
  ...
  return (
    <div>
      <div style={{marginBottom:'2rem',padding:'1.5rem',background:'linear-gradient(135deg,rgba(240,180,41,0.1),rgba(91,141,239,0.05))',border:'1px solid rgba(240,180,41,0.2)',borderRadius:'20px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
          <h1 style={{fontSize:'24px',fontWeight:'800',color:'#1C1917'}}>Organisations</h1>
          <p style={{color:'#A8A29E',fontSize:'13px',marginTop:'3px'}}>{orgs.length} organisations · {orgs.filter(o=>o.status==='Actif').length} actives · {orgs.reduce((a,o)=>a+o.users,0)} utilisateurs</p>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'1rem'}}>
        {filtered.map(org=>(
          <div key={org.id} style={{background:'#FFFFFF',border:'1px solid rgba(28,25,23,0.07)',borderRadius:'18px',padding:'1.5rem',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'1.25rem',marginTop:'4px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                ...
                <div>
                  <div style={{fontWeight:'700',fontSize:'16px',color:'#1C1917'}}>{org.name}</div>
                  <div style={{fontSize:'11px',color:'#A8A29E'}}>/{org.slug} · {org.email}</div>
                </div>
              </div>
              <div style={{display:'flex',gap:'5px',flexDirection:'column',alignItems:'flex-end'}}>
                ...
                <span style={{fontSize:'11px',fontWeight:'600',padding:'3px 10px',borderRadius:'20px',background:org.status==='Actif'?'rgba(34,212,168,0.15)':'rgba(240,90,90,0.1)',color:org.status==='Actif'?'#00BFA5':'#F05A5A'}}>{org.status}</span>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'0.6rem',marginBottom:'1.25rem'}}>
              {[{l:'Utilisateurs',v:org.users,c:'#5B8DEF'},{l:'Cours',v:org.courses,c:'#00BFA5'},{l:'Créée',v:org.created,c:'#FFB300',s:true}].map(s=>(
                <div key={s.l} style={{background:'#FAF9F7',borderRadius:'10px',padding:'10px',textAlign:'center'}}>
                  ...
                  <div style={{fontSize:'10px',color:'#A8A29E',textTransform:'uppercase',letterSpacing:'0.5px'}}>{s.l}</div>
                </div>
              ))}
            </div>
      ...
      {modal==='form'&&(
        <div style={{position:'fixed',inset:0,background:'#FAF9F7',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'1rem'}}>
          <div style={{background:'#FFFFFF',border:'1px solid rgba(91,141,239,0.25)',borderRadius:'20px',padding:'2rem',width:'100%',maxWidth:'520px'}}>
            <h2 style={{fontSize:'20px',fontWeight:'800',color:'#1C1917',marginBottom:'1.5rem'}}>{sel?'✏️ Modifier':'+ Nouvelle organisation'}</h2>
      ...
      {modal==='settings'&&(
        <div style={{position:'fixed',inset:0,background:'#FAF9F7',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'1rem'}}>
          <div style={{background:'#FFFFFF',border:'1px solid rgba(240,180,41,0.25)',borderRadius:'20px',padding:'2rem',width:'100%',maxWidth:'520px'}}>
            <h2 style={{fontSize:'20px',fontWeight:'800',color:'#1C1917',marginBottom:'1.5rem'}}>⚙️ Paramètres — {sel?.name}</h2>
            <div style={{display:'flex',flexDirection:'column',gap:'1rem',marginBottom:'1.5rem'}}>
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 14px',background:'#FAF9F7',border:'1px solid rgba(28,25,23,0.07)',borderRadius:'10px'}}>
                <div><div style={{fontWeight:'600',fontSize:'14px',color:'#1C1917'}}>SSO SAML 2.0</div><div style={{fontSize:'12px',color:'#A8A29E'}}>Authentification entreprise</div></div>
                <span style={{background:'rgba(240,180,41,0.15)',color:'#FFB300',fontSize:'11px',fontWeight:'700',padding:'3px 10px',borderRadius:'20px'}}>Pro+</span>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 14px',background:'#FAF9F7',border:'1px solid rgba(28,25,23,0.07)',borderRadius:'10px'}}>
                <div><div style={{fontWeight:'600',fontSize:'14px',color:'#1C1917'}}>Marque blanche</div><div style={{fontSize:'12px',color:'#A8A29E'}}>Logo et couleurs personnalisés</div></div>
                <span style={{background:'rgba(240,180,41,0.15)',color:'#FFB300',fontSize:'11px',fontWeight:'700',padding:'3px 10px',borderRadius:'20px'}}>Enterprise</span>
              </div>
            </div>
            <div style={{display:'flex',gap:'10px',justifyContent:'flex-end'}}>
              <button onClick={()=>{setModal(null);setSel(null)}} style={btnCancel}>Annuler</button>
              <button onClick={save} style={{...btnPrimary,background:'linear-gradient(135deg,#FFB300,#F97316)'}}>Sauvegarder</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal==='delete'&&(
        <div style={{position:'fixed',inset:0,background:'#FAF9F7',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}}>
          <div style={{background:'#FFFFFF',border:'1px solid rgba(240,90,90,0.3)',borderRadius:'20px',padding:'2.5rem',textAlign:'center',maxWidth:'400px',margin:'1rem'}}>
            ...
            <p style={{color:'#A8A29E',fontSize:'13px',marginBottom:'2rem',lineHeight:'1.6'}}>Action irréversible. Tous les {sel?.users} utilisateurs et {sel?.courses} cours seront supprimés définitivement.</p>
