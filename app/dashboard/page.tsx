...
const kpis = [
  { label: 'Cours en cours',  value: '4',      icon: '📚', bg: 'var(--turq-50)',   color: 'var(--turq-700)',   grad: 'var(--grad-ia)',      delta: '+1 ce mois',  bar: null },
  { label: 'Progression moy', value: '68%',    icon: '📈', bg: 'var(--gold-50)',   color: 'var(--gold-700)',   grad: 'var(--grad-ia)',      delta: '+5% semaine', bar: 68 },
  { label: 'Score moyen',     value: '82/100', icon: '🏆', bg: 'var(--orange-50)', color: 'var(--orange-700)', grad: 'var(--grad-energie)', delta: 'Top 15%',     bar: 82 },
  { label: 'Heures semaine',  value: '12h',    icon: '⏱',  bg: 'var(--gold-50)',   color: 'var(--gold-700)',   grad: 'var(--grad-energie)', delta: 'Obj : 15h',   bar: 80 },
]

const courses = [
  { title: 'Data Science avec Python',  progress: 72, cat: 'Tech',        color: 'var(--turq-700)',   bg: 'var(--turq-50)',   grad: 'var(--grad-ia)',        done: 17, total: 24 },
  { title: 'Marketing Digital Afrique', progress: 45, cat: 'Business',    color: 'var(--orange-700)', bg: 'var(--orange-50)', grad: 'var(--grad-signature)', done: 8,  total: 18 },
  { title: 'Leadership & Management',   progress: 30, cat: 'Soft Skills', color: 'var(--gold-700)',   bg: 'var(--gold-50)',   grad: 'var(--grad-energie)',   done: 4,  total: 12 },
]
...
// §4 bis — aplats PLEINS + texte blanc (jamais -50 pour les cartes principales)
const featureCards = [
  { bg: 'var(--gold)',           label: 'Mes cours',    emoji: '📚', sub: '4 en cours',        href: '/cours' },
  { bg: 'var(--turq)',           label: 'Classes live',  emoji: '🎥', sub: 'Prochaine : demain', href: '/live' },
  { bg: 'var(--grad-signature)', label: 'AI Tutor',      emoji: '✦',  sub: 'Disponible 24/7',   href: '/tutor' },
  { bg: 'var(--orange)',         label: 'Marketplace',   emoji: '🛒', sub: '200+ formations',   href: '/market' },
  { bg: 'var(--grad-energie)',   label: "Pass'BAC",      emoji: '🎓', sub: '8 matières',        href: '/apprenant/passbac' },
]
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1100px' }}>
      ...
      {/* Feature cards — §4 bis : aplats pleins, texte blanc */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(185px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        {featureCards.map(({ bg, label, emoji, sub, href }) => (
          <div key={label} onClick={() => router.push(href)} style={{
            borderRadius: '18px', overflow: 'hidden', cursor: 'pointer',
            boxShadow: 'var(--sh-2)', transition: 'transform .15s, box-shadow .15s',
            background: 'var(--surface)', border: '1px solid var(--line)',
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = 'var(--sh-3)' }}
          onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLDivElement).style.boxShadow = 'var(--sh-2)' }}
          >
            {/* Bandeau APLAT PLEIN — couleur franche, texte blanc */}
            <div style={{ background: bg, padding: '1rem 1.125rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--sans)', fontWeight: 800, fontSize: '13px', color: '#fff' }}>{label}</span>
              <span style={{ fontSize: '22px', filter: 'drop-shadow(0 1px 3px rgba(0,0,0,.2))' }}>{emoji}</span>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        {kpis.map(({ label, value, icon, bg, color, grad, delta, bar }) => (
          <div key={label} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '16px', padding: '1.25rem', boxShadow: 'var(--sh-1)' }}>
            ...
            <div style={{ fontFamily: 'var(--sans)', fontSize: '12px', color: 'var(--ink-soft)', marginTop: '3px', fontWeight: 500, marginBottom: bar !== null ? '10px' : 0 }}>{label}</div>
            {bar !== null && (
              <div style={{ height: '8px', background: 'var(--line)', borderRadius: '99px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${bar}%`, background: grad, borderRadius: '99px', transition: 'width 0.6s ease' }} />
              </div>
            )}
          </div>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '1.25rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '20px', padding: '1.5rem', boxShadow: 'var(--sh-1)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {courses.map(({ title, progress, cat, color, bg, grad, done, total }) => (
              ...
              >
                ...
                {/* Barre 8px gradient plein */}
                <div style={{ height: '8px', background: 'var(--line)', borderRadius: '99px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${progress}%`, background: grad, borderRadius: '99px', transition: 'width 0.5s' }} />
                </div>
              </div>
            ))}
          </div>
