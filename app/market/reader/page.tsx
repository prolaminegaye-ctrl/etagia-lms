...
function ReaderContent() {
  ...
  // ─── PAS D'ACCÈS → PAYWALL ───
  if (!hasAccess) return (
    <div style={{ minHeight: '100vh', background: 'var(--canvas)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ textAlign: 'center', maxWidth: '480px', width: '100%' }}>
        ...
        <p style={{ color: 'var(--ink-mut)', lineHeight: 1.7, marginBottom: '1.5rem', fontSize: '14px' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '16px', padding: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          {product.chapitres && product.chapitres.length > 0 && (
            <div style={{ textAlign: 'left', background: 'var(--bg-secondary)', borderRadius: '10px', padding: '10px 12px', marginBottom: '12px' }}>
              ...
              {product.chapitres.slice(0, 4).map((c: string, i: number) => (
                <div key={i} style={{ fontSize: '12px', color: 'var(--ink-mut)', padding: '3px 0', display: 'flex', gap: '8px' }}>
                  <span style={{ color: 'var(--orange)', fontWeight: '700', minWidth: '16px' }}>{i + 1}.</span>{c}
                </div>
              ))}
          ...
          <div style={{ fontSize: '24px', fontWeight: '900', color: 'var(--orange)', fontFamily: 'var(--font-display)' }}>
            {(product.price / 100).toLocaleString('fr')} FCFA
          </div>
        </div>
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
          <button onClick={() => router.push('/market')} style={{ background: 'var(--orange)', border: 'none', borderRadius: '10px', padding: '12px 28px', color: '#fff', fontWeight: '700', fontSize: '14px', cursor: 'pointer', fontFamily: 'var(--font-display)' }}>
            🛒 Acheter & Débloquer
          </button>
          <button onClick={() => router.back()} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '10px', padding: '12px 20px', color: 'var(--ink-mut)', fontWeight: '500', fontSize: '14px', cursor: 'pointer' }}>
            Retour
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--canvas)', display: 'flex', flexDirection: 'column' }}>
      {/* TOAST */}
      {toast && (
        <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', zIndex: 9999, background: 'var(--surface)', border: '1px solid rgba(74,127,245,0.4)', borderRadius: '12px', padding: '12px 20px', fontSize: '14px', fontWeight: '600', boxShadow: '0 8px 32px rgba(0,0,0,0.4)', color: 'var(--ink)' }}>
          {toast}
        </div>
      )}

      {/* Sticky header */}
      <div style={{ padding: '0.9rem 2rem', borderBottom: '1px solid var(--line)', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {hasPdf && (
            <div style={{ display: 'flex', background: 'var(--bg-secondary)', borderRadius: '8px', border: '1px solid var(--line)', overflow: 'hidden' }}>
              <button onClick={() => setPdfMode('embed')} style={{ padding: '6px 12px', fontSize: '11px', fontWeight: '600', cursor: 'pointer', border: 'none', background: pdfMode === 'embed' ? 'var(--orange)' : 'transparent', color: pdfMode === 'embed' ? '#fff' : 'var(--text-muted)' }}>
                📄 PDF
              </button>
              <button onClick={() => setPdfMode('text')} style={{ padding: '6px 12px', fontSize: '11px', fontWeight: '600', cursor: 'pointer', border: 'none', background: pdfMode === 'text' ? 'var(--orange)' : 'transparent', color: pdfMode === 'text' ? '#fff' : 'var(--text-muted)' }}>
                📝 Texte
              </button>
            </div>
          )}
          ...
          {(!hasPdf || pdfMode === 'text') && (
            <>
              ...
              <span style={{ fontSize: '11px', color: 'var(--ink-mut)', minWidth: '22px', textAlign: 'center' }}>{fontSize}</span>
              <button onClick={() => setFontSize(s => Math.min(22, s + 1))} style={sizeBtn}>A+</button>
            </>
          )}
          <button onClick={handleDownload} style={{ background: 'var(--orange)', border: 'none', borderRadius: '8px', padding: '7px 16px', color: '#fff', fontSize: '12px', fontWeight: '700', cursor: 'pointer', fontFamily: 'var(--font-display)', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {hasPdf && pdfMode === 'embed' ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '1.5rem', gap: '1rem' }}>
          ...
          {/* PDF iframe */}
          <div style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '12px', overflow: 'hidden' }}>
            <iframe
        ...
        <div style={{ flex: 1, maxWidth: '820px', margin: '0 auto', width: '100%', padding: '2.5rem 2rem' }}>
          ...
          {/* Hero */}
          <div style={{ textAlign: 'center', marginBottom: '3rem', paddingBottom: '2.5rem', borderBottom: '1px solid var(--line)' }}>
            ...
            <p style={{ color: 'var(--ink-mut)', fontSize: '14px', margin: '0 0 1.2rem', lineHeight: 1.6 }}>{product.desc}</p>
          ...
          {product.chapitres && product.chapitres.length > 0 && (
            <div style={{ marginBottom: '2rem', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '8px' }}>
                {product.chapitres.map((c: string, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', padding: '8px 10px', background: 'var(--bg-secondary)', borderRadius: '8px' }}>
                    <span style={{ fontWeight: '800', color: 'var(--orange)', fontSize: '13px', minWidth: '22px' }}>{i + 1}.</span>
                    <span style={{ fontSize: '13px', color: 'var(--ink-mut)', lineHeight: 1.4 }}>{c}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Body content */}
          <div style={{ fontSize: `${fontSize}px`, lineHeight: 1.95, color: 'var(--ink)', whiteSpace: 'pre-wrap', fontFamily: 'var(--font-body)', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '16px', padding: '2rem' }}>
            {product.longDesc}
          </div>
          ...
          <div style={{ marginTop: '2.5rem', padding: '1.75rem', background: 'linear-gradient(135deg, rgba(74,127,245,0.07), rgba(32,212,168,0.04))', border: '1px solid rgba(74,127,245,0.2)', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <button onClick={handleDownload} style={{ background: 'linear-gradient(135deg, var(--orange), #D4A017)', border: 'none', borderRadius: '10px', padding: '12px 28px', color: '#fff', fontWeight: '700', fontSize: '14px', cursor: 'pointer', fontFamily: 'var(--font-display)', whiteSpace: 'nowrap' }}>
...
const backBtn: React.CSSProperties = {
  ...
  padding: '7px 14px', color: 'var(--ink-mut)', fontSize: '12px', cursor: 'pointer',
  fontFamily: 'var(--font-display)', fontWeight: '500',
}
const sizeBtn: React.CSSProperties = {
  background: 'var(--bg-secondary)', border: '1px solid var(--line)', borderRadius: '6px',
  padding: '4px 9px', color: 'var(--ink-mut)', fontSize: '11px', cursor: 'pointer', fontWeight: '600',
}
